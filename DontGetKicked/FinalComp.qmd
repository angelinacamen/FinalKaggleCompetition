...
randforest_models <- rf_wf %>%
  tune_grid(resamples=folds,
            grid=rand_forest_tuning_grid,
            metrics=metric_set(gain_capture), # like roc_auc but for a gain curve
            control = untunedModel, verbose = TRUE)

lightGBM_models <- lgb_workflow %>%
  tune_grid(resamples=folds,
